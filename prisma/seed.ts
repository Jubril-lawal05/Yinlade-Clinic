...
async function main() {
  // Make the seed idempotent in CI/dev: reset tables first.
  ...
  const staffByLegacy = new Map<string, { id: string; role: any; name: string }>();
  ...
  const patientByLegacy = new Map<string, { id: string; name: string }>();
  ...
  const clinicalSeed = [
    {
      ...
      extraOral:
        "No swelling. Lymph nodes non-palpable. TMJ mild clicking left side.",
  ...
  for (const c of clinicalSeed) {
    ...
    const clinical = await prisma.clinicalRecord.create({
      data: {
        patientId,
        odontogram: c.odontogram as any,
    ...
    // Clear existing notes for idempotent seeding
    await prisma.treatmentNote.deleteMany({ where: { clinicalRecordId: clinical.id } });
  ...
  // Appointments
  const apptsSeed = [
    { legacyId: "A001", pid: "P001", date: "2026-03-25", time: "09:00", type: "Cleaning", status: "confirmed", notes: "Regular checkup" },
    { legacyId: "A002", pid: "P002", date: "2026-03-20", time: "10:30", type: "Root Canal", status: "confirmed", notes: "Follow-up" },
    { legacyId: "A003", pid: "P003", date: "2026-04-01", time: "14:00", type: "Filling", status: "pending", notes: "" },
    { legacyId: "A004", pid: "P004", date: "2026-03-22", time: "11:00", type: "Extraction", status: "confirmed", notes: "Wisdom tooth" },
    { legacyId: "A005", pid: "P005", date: "2026-03-28", time: "15:30", type: "Braces Consult", status: "pending", notes: "" },
    { legacyId: "A006", pid: "P001", date: "2026-03-18", time: "09:00", type: "Emergency", status: "completed", notes: "Toothache" },
  ] as const;
      ...
      data: {
        ...
        items: {
          create: inv.items.map((it) => ({
            description: it.d,
            amount: new Prisma.Decimal(it.a),
          })),
        },
      },
    });
  ...
  // Seed communications messages (store-only for now)
  const messagesSeed = [
    {
      pid: "P001",
      type: "Reminder",
